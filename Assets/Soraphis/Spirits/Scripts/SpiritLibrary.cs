            ...
            for (int y = 1; y < grid.GetUpperBound(1); y++) {
                ...
                for (int x = 3; x < grid.GetUpperBound(0); x++) {
                    if(grid[x, 0] != "") { 
                        ...
                        if(field.FieldType == typeof(int)) {    field.SetValue(t, int.Parse(grid[x, y]));   }
                        else if (field.FieldType == typeof(float)) { field.SetValue(t, float.Parse(grid[x, y])); }
                        else if (field.FieldType == typeof(bool)) { field.SetValue(t, bool.Parse(grid[x, y])); }
                        else { field.SetValue(t, grid[x, y]); } // string
